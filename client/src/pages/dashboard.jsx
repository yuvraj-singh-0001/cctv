...
function Dashboard() {
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          ...
          {/* Search bar and Refresh button for larger screens */}
          {screenSize !== 'xs' && (
            <motion.div
            ...
            >
              ...
              {/* Refresh Button for PC/Desktop */}
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={handleRefresh}
                className="hidden lg:flex items-center gap-2 px-3 py-2 rounded-lg text-white transition-colors"
      ...
      <motion.div 
        ...
        className="mb-4 mt-6 sm:mb-6 md:mb-8 bg-white rounded-lg sm:rounded-xl shadow-sm border border-gray-200 overflow-hidden"
      >
        <div className="border-b border-gray-200 px-3 sm:px-4 md:px-6 py-2 sm:py-3 md:py-4 flex flex-col xs:flex-row justify-between items-start xs:items-center gap-2 xs:gap-0">
          <h2 className="text-base sm:text-lg font-medium" style={{color: 'rgb(7,72,94)'}}>
    ...
    {/* Floating Refresh Button - Only for mobile (hidden on lg screens and above) */}
    <motion.button
      whileHover={{ scale: 1.05 }}
      whileTap={{ scale: 0.95 }}
      onClick={handleRefresh}
      className="fixed right-4 bottom-5 lg:hidden z-40 p-3 rounded-full shadow-lg bg-[rgb(7,72,94)] text-white hover:bg-[rgb(6,60,78)] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[rgb(7,72,94)]"
      aria-label="Refresh data"
      title="Refresh"
    >
      <RefreshCw size={20} />
