...
function Sidebar({ activeTab, setActiveTab, isCollapsed, setIsCollapsed }) {
  ...
  return (
    <>
      {/* Mobile Top Bar */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-14 bg-white border-b flex items-center justify-between px-3 z-30" style={{borderColor: 'rgb(7,72,94)'}}>
        <button onClick={() => setMobileOpen(true)} className="p-2 rounded-md hover:bg-gray-100 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[rgb(7,72,94)]">
      ...
      {/* Mobile Overlay */}
      {mobileOpen && <div className="md:hidden fixed inset-0 bg-black/30 z-20" onClick={closeMobile} aria-hidden="true" />}

      {/* Mobile Sidebar Drawer */}
      <div className={`md:hidden fixed top-0 left-0 h-full w-64 bg-white shadow-lg transition-transform duration-300 z-30 border-r ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`} style={{borderColor: 'rgb(7,72,94)'}} role="dialog" aria-modal="true" aria-label="Mobile Sidebar">
        <div className="flex flex-col h-full">
          <div className="h-14 px-4 border-b flex items-center justify-between" style={{borderColor: 'rgb(7,72,94)', backgroundColor: 'rgb(205,225,230)'}}>
            <div className="flex items-center">
              <img src={Logo} alt="CCTV Manager Logo" className="w-7 h-7 mr-2 object-contain" />
              <h2 className="text-lg font-semibold" style={{color: 'rgb(7,72,94)'}}>Menu</h2>
            </div>
            <button onClick={closeMobile} className="p-2 rounded-full hover:bg-gray-100" style={{color: 'rgb(7,72,94)'}}>
              <ChevronLeft size={20} />
            </button>
          </div>
          ...
          <nav className="flex-1 p-3 flex flex-col" role="navigation" aria-label="Sidebar">
            <ul className="space-y-1 flex-1">
              {menuItems.map((item) => {
                ...
                return (
                  <li key={item.id}>
                    <button
                      onClick={() => { setActiveTab(item.id); navigate(item.path); closeMobile(); }}
            ...
            <div className="mt-4 pt-4 border-t" style={{borderColor: 'rgba(7,72,94,0.2)'}}>
              <button onClick={handleLogout} className="w-full flex items-center rounded-lg p-3.5 transition-colors text-[rgb(7,72,94)] hover:bg-red-50 hover:text-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-400">
      ...
      {/* Desktop Sidebar */}
      <div className={`hidden md:block fixed top-0 left-0 h-full bg-white shadow-lg transition-all duration-300 z-10 ${isCollapsed ? 'md:w-20' : 'md:w-64'}`} style={{borderRight: `1px solid rgba(22, 105, 132, 1)`}}>
        <div className="flex flex-col h-full">
          <div className="p-4 border-b flex items-center justify-between" style={{borderColor: 'rgb(7,72,94)', backgroundColor: 'rgb(205,225,230)'}}>
            {!isCollapsed ? (
              <div className="flex items-center">
            ...
            <div className="flex items-center">
              <button onClick={toggleSidebar} className="p-1 rounded-full hover:bg-gray-100" style={{color: 'rgb(7,72,94)'}}>
          ...
          <nav className="flex-1 p-4 flex flex-col" role="navigation" aria-label="Sidebar">
            ...
            <div className="mt-4 pt-4 border-t" style={{borderColor: 'rgba(7,72,94,0.2)'}}>
              <button onClick={handleLogout} className="w-full flex items-center rounded-lg p-3.5 transition-colors text-[rgb(7,72,94)] hover:bg-red-300 hover:text-red-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-400">
                <LogOut size={20} className="shrink-0" />
                {!isCollapsed && <span className="ml-3">Logout</span>}
              </button>
            </div>
